...
Module TrippleDescspSample
    Sub Main()
        Try
            ...
            Dim sData As String = "Hope this works during the demo"
            Dim FileName As String = "Encrypted.txt"

            ' Protect text to a file using the file name, key, and IV.
            EncryptTextToFile(sData, FileName, tDESalg.Key, tDESalg.IV)

            Dim keyEncryptor as new KeyProtector
            Dim entropy(15) As Byte
            Dim provider As New RNGCryptoServiceProvider()
            provider.GetBytes(entropy)
            Dim bytesEncrypted as Integer = keyEncryptor.Protect(entropy, tDESalg.Key, "EncryptedKey.dat")
            Dim decryptedKey As Byte() = keyEncryptor.UnProtect(entropy, "EncryptedKey.dat", bytesEncrypted)
            ' Decrypt the text from a file using the file name, key, and IV.
            Dim Final As String = DecryptTextFromFile(FileName, decryptedKey, tDESalg.IV)
